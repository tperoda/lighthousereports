...
if (argv.from && argv.to) {
  ...
  } else if (!fs.existsSync(`./lighthouse-reports/${dirName}`)) {
    fs.mkdirSync(`./lighthouse-reports/${dirName}`);
  }
  ...
  launchChromeAndRunLighthouse(urls[dirName]).then(results => {
    ...
    // If the path has changed, this block needs altering as well
    fs.writeFile(
      `lighthouse-reports/${dirName}/${results.js["fetchTime"].replace(/:/g, "_")}.json`,
      results.json,
      err => {
        if (err) throw err;
      }
